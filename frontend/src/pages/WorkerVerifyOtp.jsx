...
export default function WorkerVerifyOtp() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      await postJson("/workers/verify-otp/", { otp: otpString });
      navigate("/");
    } catch (err) {
    ...
    <div className="app-container">
      <nav className="navbar" style={{ 
        ...
        <div className="nav-links" style={{ gap: 'var(--s-6)', marginLeft: 'var(--s-8)', background: 'rgba(15, 23, 42, 0.05)', padding: 'var(--s-2) var(--s-4)', borderRadius: '2rem', backdropFilter: 'blur(10px)' }}>
          ...
          <Link to="/workers/login" style={{ color: 'var(--primary)', fontSize: '0.85rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Login</Link>
        </div>
      </nav>
      ...
      <main className="content-wrapper animate-fade-in mesh-bg" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh' }}>
        <div className="form-card glass-panel" style={{ maxWidth: '440px', margin: 0, padding: 'var(--s-12)' }}>
          <div className="text-center mb-10">
            <span className="status-pill mb-4" style={{ backgroundColor: 'rgba(5, 150, 105, 0.1)', color: 'var(--success)' }}>Professional Access</span>
            <h2 className="form-title title-font" style={{ fontSize: '2.5rem', marginBottom: 'var(--s-2)' }}>Verify Pro Identity</h2>
            <p className="form-subtitle">
              Securely access your workspace by entering the 4-digit code dispatched to your account.
            </p>
          </div>
          ...
          <form onSubmit={onSubmit}>
            ...
            <button type="submit" disabled={loading} style={{ backgroundColor: '#059669', color: 'white', padding: '1.125rem', width: '100%', fontSize: '1.1rem' }}>
              {loading ? <div className="spinner"></div> : "Authorize Workspace Access"}
            </button>
            
            {error && <p className="error-msg text-center mt-6">{error}</p>}
            
            <p className="text-center mt-10" style={{ fontSize: '0.9rem' }}>
              Authentication issue? <button type="button" onClick={() => navigate('/workers/login')} style={{ background: 'none', border: 'none', color: '#059669', fontWeight: 700, cursor: 'pointer', padding: 0 }}>Return to Login</button>
            </p>
          </form>
        </div>
      </main>
